...
EXTRA = {
    ...
    "PB-097": ("PARTIAL", "One adapter carries both meters, so one meter cannot be unplugged alone; "
                          "offline/online handling verified in PB-106"),
...
summ.append([])
summ.append(["Setup", "PC pandabox-tester (BLE) -> PandaBox (GD32F305, bench build) -> RS232 Port 2 -> USB-RS232 COM7 -> "
                      "LCR simulator (meter 1 LCR-II node 1, meter 2 LCR.iQ node 2)"])
summ.append(["Automated checks", f"{sum(x['ok'] for x in run)}/{len(run)} passed (tools/tracker_suite.py)"])
summ.append(["Tester happy_flow.csv", "63/63 OK (pandabox-tester runner, validator v1.86)"])
summ.append([])
