...
/* wire exchange: request frame -> response frame (0 bytes = no answer / timeout) */
...
int lcr_issue(int port, uint8_t cmd)
{
    ...
    if(lcr_xfer(port, lcr_port[port].node, req, 2U, &f) < 2) {
        return -1;
    }
...
static int set_volume_field(int port, uint8_t fld, int32_t v)
{
    ...
    if(lcr_xfer(port, lcr_port[port].node, req, 6U, &f) < 2) {
        return -1;
    }
    return f.data[0];
}
